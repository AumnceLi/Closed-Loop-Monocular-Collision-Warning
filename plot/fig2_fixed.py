...
def filter_subset_rows(df: pd.DataFrame, subset_name: str) -> pd.DataFrame:
    """
    Keep only the rows that truly belong to the requested subset.

    The uploaded batch summary CSVs may contain extra rows or blank-label rows.
    If dataset_subset_label exists, filter by it explicitly. Otherwise, keep all rows.
    """
    out = df.copy()
...
def plot_figure4_coupled_scatter(caution_df: pd.DataFrame, danger_df: pd.DataFrame) -> None:
    ...
    present_outcomes = list(dict.fromkeys(pd.concat([caution_plot["outcome"], danger_plot["outcome"]]).tolist()))
    legend_elements = []
    if "executed" in present_outcomes:
        legend_elements.append(
            Line2D([0], [0], marker='o', color=PALETTE['blue'], label='Executed', markersize=5, linestyle='None')
        )
    if "rejected" in present_outcomes:
        legend_elements.append(
            Line2D([0], [0], marker='x', color=PALETTE['orange'], label='Rejected', markersize=6, linestyle='None')
        )
    if "vetoed" in present_outcomes:
        legend_elements.append(
...
def main() -> None:
    ...
    if len(safe_df) == 0 or len(caution_df) == 0 or len(danger_df) == 0:
        raise RuntimeError("Subset filtering removed all rows for at least one subset. Please inspect dataset_subset_label values.")
